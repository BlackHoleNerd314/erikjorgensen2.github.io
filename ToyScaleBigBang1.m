...
%% Energy Densities of Thermal Radiation
  ...
  Nneutrinos = 6*(3/4)*E2N*(T_neutrinos/MeV)^3;
  Nphotons = gPhotons*E2N;
...
ind4 = 1;
ansWeakon = [int2str([(2*NWeakons(ind4,1)/number)*(10^15)]),name3(ind4,:),int2str([NWeakons(ind4,2)/MeV0])]
ind4 = 2;
ansWeakon = [int2str([(NWeakons(ind4,1)/number)*(10^15)]),name3(ind4,:),int2str([NWeakons(ind4,2)/MeV0])]
%
o_y
o_v
T_qqqM
...
time0 = s;
radius1 = c0*time0;
radius2 = (2*G0*(h0/(2*pi))*time0^3)^(1/5);
mass10 = radius1*(c0^2)/(2*G0);
mass11 = (h0/(2*pi))/(radius1*c0);
mass22 = 1/sqrt((2*G0/radius2)/(h0/(2*pi*time0)));
mass3 = eV*(1.602176634/10^19)/(c0^2);
mass0 = 1.055*2.72548*kB/(c0^2);
radius3 = a0*(((10^15)/(number))*((0.01^3)/411))^(1/3);
radius0 = radius3/a0;
mass4 = [mass10,mass11,mass22,mass3]/mass0;
radius4 = [radius1,radius1,radius2,radius3]/radius0;
time4 = time0/(4.35456*10^17);
radius4;
mass4;
