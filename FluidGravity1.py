...
def lennard_jones_force_cell_list(positions, L, rcut, epsilon, sigma):
    ...
    # This kernel iterates through neighboring cells and computes forces
    # We use a raw kernel to optimize performance
    ...
    void lennard_jones_cell_kernel(
        ...
        const int* __restrict__ sort_perm, // Fix: Added sort_perm to kernel arguments
        double* __restrict__ forces) {
    ...
    lj_kernel((blocks_per_grid,), (threads_per_block,), (
        ...
        cell_start_indices,
        sorted_positions,
        sort_perm,  # Fix: Added sort_perm to the launch arguments
        forces_unsorted
    ))
...
def gravitational_force_fourier(positions, L, G, grid_size):
    ...
    # This is a bit tricky with periodic boundary conditions
...
def main():
    ...
    # Positions are random within the box [0, L)
    # Velocities are initialized to zero
    positions = L * cp.random.rand(N, 3, dtype=cp.float64)
