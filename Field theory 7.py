...
while running:
    ...
    # -----------------------------
    PsiT += dt * (LapPsi - M**2 * Psi + G * Psi * Phi - Psi * 0*np.abs(Psi)**2)
    Psi += dt * PsiT
    ...
    phi0 = np.fft.fft2(Phi)
    fs = int(N/2)
    phi0[0,0] = 0
    Phi = np.fft.ifft2(phi0)


    # -----------------------------
    # Visualization (sum over z-axis)
    # -----------------------------
    density2d = np.abs(Psi)**2
    img1 = normalize_density(density2d)
    density2d = np.abs(Phi)**2
