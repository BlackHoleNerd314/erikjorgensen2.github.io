...
GRID_SIZE = 1024
NUM_PARTICLES = 65536  # Try increasing this to see the performance of the GPU
BOX_SIZE = 4096  # The size of our 2D simulation box (float)
TIME_STEP = 0.02
...
# Damping parameter
K_DAMPING = 0.0001  # A positive value for the damping strength
...
# --- CUDA Kernel for Lennard-Jones and Damping Force with Cell Lists ---
# We pass the velocity array and the damping constant to the kernel.
# The velocity damping force is calculated as F_damp = 0.5 * K_DAMPING * (v_neighbor - v_current).
# This force brings each particle's velocity closer to the pair's average velocity
# while conserving momentum (F_damp_a = -F_damp_b).
...
void cell_list_lj_kernel(
    ...
    double p_vel_x = vel[tid * 2];
    double p_vel_y = vel[tid * 2 + 1];
    ...
    for (int i_x = -1; i_x <= 1; ++i_x) {
        for (int i_y = -1; i_y <= 1; ++i_y) {
            ...
            for (int j = start_idx; j < end_idx; ++j) {
                ...
                double q_vel_x = vel[neighbor_tid * 2];
                double q_vel_y = vel[neighbor_tid * 2 + 1];
                ...
                if (r2 < cutoff2) {
                    ...
                    // Damping force calculation (brings velocities to their average)
                    // This force is F_damp = 0.5 * k_damping * (v_neighbor - v_current)
                    // and is symmetric, so the total momentum is conserved.
    ...
    // Add computed forces to the global force array (which is 'acc' in the Python code)
...
class NBodySimulation:
    def __init__(self):
        ...
        pygame.display.set_caption("2D N-Body Simulation (Lennard-Jones + Gravity + Damping)")
...
def calculate_forces(self):
    """Calculates the total force (LJ + Gravity + Damping) on all particles."""
    # Reset accelerations (force per unit mass stored in acc)
    self.acc.fill(0.0)

    # --- 1. Lennard-Jones & Damping Force (short-range) using Cell Lists ---
    ...
    # Call the CUDA kernel with the correct particle_cell_idx and new parameters
    cell_list_lj_kernel((blocks,), (threads_per_block,), (
        self.pos, self.acc, self.vel, particle_cell_idx, cell_start, cell_count,
